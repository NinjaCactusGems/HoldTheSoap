...
- **All timed transitions run on the single DO alarm** — never `setTimeout`. `armAlarm()` schedules the next due event (`readyEndsAt`, `winnerEndsAt`, or during holding the earliest of bot drops, the abandonment deadline, and the `holdEndsAt` backstop); `onAlarm()` dispatches purely on persisted state, so stale/early fires degrade to a re-arm.
- **Watchdog:** players still in the round send an `alive` heartbeat every ~60 s (`ALIVE_INTERVAL_MS` in `Lobby.tsx`) — necessary because a perfectly steady phone sends nothing else, and the `'k'` keepalive never reaches the DO. The round resolves to "no one" once *everyone* has been silent for `HOLD_ABANDON_MS` (2 min — i.e. the remaining players vanished without a clean close), or at the `MAX_HOLD_DURATION_MS` absolute cap (60 min) regardless. Legitimate marathon rounds with live players keep going.
